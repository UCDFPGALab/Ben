...
void setup()
{
  ...
  correctData = reverseBits8(correctData);
  ...
  if(currentRun < numRuns)
  {
    ...
    if(addressSteps < NUM_ADDRESSES)
    {
      ...
      if(writing)
      {
        ...
        writeData(reverseAddress(addressInt), correctData);
      ...
      else
      {
        ...
        //oldData = dataInt;
        // Read an address
        //Serial.println(toBinary(reverseAddress(addressInt), 19));
        dataInt = readData(reverseAddress(addressInt));
        // Compare the output
...
char reverseBits8(unsigned char data)
{
...
int reverseAddress(int data)
{
  String holder = toBinary(data, 19);
  String holderII = holder.substring(0,7) + holder.substring(9, 18);
  int valueToReverse = 0;
  int finalValue = 0;
  int multiple = 1;
  for(int i = 0; i < holderII.length();  i++, multiple = multiple*2)
  {
    valueToReverse = valueToReverse + multiple*(holderII.charAt(holderII.length() - i - 1) - '0');
  }
  data = valueToReverse; 
  data = reverseBits16(short(valueToReverse));
  holderII = (toBinary(data,16)).substring(0,7) + "00" + (toBinary(data,16)).substring(7,16) + "0";
  multiple = 1;
  for(int i = 0; i < holderII.length(); i++, multiple = multiple*2)
  {
    finalValue = finalValue + multiple *(holderII.charAt(holderII.length()- i - 1) - '0');
  }
  return finalValue;
}

unsigned short reverseBits16(unsigned short data)
{
  data = ((data >> 1) & 0x5555) | ((data << 1) & 0xaaaa);
  data = ((data >> 2) & 0x3333) | ((data << 2) & 0xcccc);
  data = ((data >> 4) & 0xf0f) | ((data << 4) & 0xf0f0);
  data = ((data >>8 & 0xff) | ((data << 8) & 0xffff));
  return data;
}
